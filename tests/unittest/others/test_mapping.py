...
class TestMapping(unittest.TestCase):
    ...
    def test_mapping(self):
        ...
        m = Mapping(world_size=8, rank=0, tp_size=2, pp_size=2, cp_size=2)
        self.assertEqual(len(m.tp_groups), 4)
        self.assertEqual(len(m.pp_groups), 4)
        self.assertEqual(len(m.cp_groups), 4)
        self.assertEqual(m.tp_group, [0, 1])
        self.assertEqual(m.pp_group, [0, 2])
        self.assertEqual(m.cp_group, [0, 4])
        self.assertTrue(m.is_first_pp_rank())
        self.assertFalse(m.is_last_pp_rank())
        self.assertTrue(m.is_first_cp_rank())
        self.assertFalse(m.is_last_cp_rank())
        self.assertEqual(m.prev_pp_rank(), 2)
        self.assertEqual(m.next_pp_rank(), 2)
        self.assertEqual(m.prev_cp_rank(), 4)
        self.assertEqual(m.next_cp_rank(), 4)

        m = Mapping(world_size=8, rank=7, tp_size=2, pp_size=2, cp_size=2)
        self.assertEqual(m.tp_group, [6, 7])
        self.assertEqual(m.pp_group, [5, 7])
        self.assertEqual(m.cp_group, [3, 7])
        self.assertFalse(m.is_first_pp_rank())
        self.assertTrue(m.is_last_pp_rank())
        self.assertFalse(m.is_first_cp_rank())
        self.assertTrue(m.is_last_cp_rank())
        self.assertEqual(m.prev_pp_rank(), 5)
        self.assertEqual(m.next_pp_rank(), 5)
        self.assertEqual(m.prev_cp_rank(), 3)
        self.assertEqual(m.next_cp_rank(), 3)
