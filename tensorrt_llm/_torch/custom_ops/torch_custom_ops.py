...
def attention(
    ...
    if compute_attention_stats:
        output = q.new_empty((num_tokens, num_heads * v_head_size),
                             dtype=out_dtype)
        ...
        torch.ops.trtllm.attention_inplace(
            ...
            latent_cache, q_pe, block_ids_per_seq, is_fused_qkv,
            update_kv_cache, predicted_tokens_per_seq, layer_idx, num_heads,
            num_kv_heads, head_size, tokens_per_block, max_num_requests,
            max_context_length, attention_window_size, sink_token_length,
            beam_width, mask_type, quant_mode, q_scaling,
            position_embedding_type, rotary_embedding_dim,
            ...
            mla_context_kv_cache_block_offsets, stats)
        return [output, stats]
    else:
        output = q.new_empty((num_tokens, num_heads * v_head_size),
                             dtype=out_dtype)
        torch.ops.trtllm.attention_inplace(
            ...
            qk_nope_head_dim, qk_rope_head_dim, v_head_dim,
            mrope_rotary_cos_sin, mrope_position_deltas, mla_context_paged_kv,
            mla_context_kv_cache_block_offsets, None)
